...
*`Alternate Data Streams (ADS)` - Feature that allows files to store hidden secondary metadata streams (often analyzed in forensics and malware analysis).

*`%windir%` - System environment variable pointing to the main directory where the Windows operating system is installed (typically `C:\Windows`).

*`System32` - The primary Windows system directory (`C:\Windows\System32`) containing essential system files, executables, drivers, and DLLs required for the operating system to function.
...
*`Ctrl + Shift + Esc` - Directly opens Windows Task Manager.
